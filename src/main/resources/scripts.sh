...
docker run --name=aa-svc --net=host -p 9092:9092 -v $(pwd)/aa-svc-logs:/logs -d -e JAVA_TOOL_OPTIONS='-Duser=fiu_aa_testing -Dpwd=fiu_aa_testing -DCLIENT_API_KEY=****' icredittechnocrats/aa-svc:latest
